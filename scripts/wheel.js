...
function loadLeaderboard() {
  ...
  wheelDb
    ...
    .then((snapshot) => {
      ...
      let html =
        '<table class="leaderboard-table"><thead><tr><th>#</th><th>Name</th><th>Codes</th><th>Gewinne</th></tr></thead><tbody>';
      let rank = 1;
      ...
      snapshot.forEach((doc) => {
        ...
        const rowClass = rank <= 3 ? `leaderboard-top leaderboard-top-${rank}` : "";

        html += `
          <tr class="${rowClass}">
            <td class="leaderboard-rank">${medal}</td>
            <td class="leaderboard-name">${escapeHtml(data.nickname || "Unbekannt")}</td>
            <td class="leaderboard-codes">${data.codesCracked} 🔑</td>
            <td class="leaderboard-rewards">${renderRewardBadges(data.rewards)}</td>
          </tr>
        `;
        rank++;
      });
