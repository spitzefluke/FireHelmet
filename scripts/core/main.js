...
function drawHomeFrame(time) {
  ...
  // Nebel-Wolken, langsam auf leichten Kreisbahnen driftend
  homeNebulae.forEach((n) => {
    n.angle += n.speed;
    const x = n.baseX + Math.cos(n.angle) * 60;
    const y = n.baseY + Math.sin(n.angle) * 40;

    const gradient = homeCtx.createRadialGradient(x, y, 0, x, y, n.radius);
  ...
  // Twinkelnde Sterne mit Parallax-Drift
  homeStars.forEach((star) => {
    ...
    homeCtx.globalAlpha = Math.max(0, twinkle);
    homeCtx.fillStyle = star.color;
    homeCtx.shadowColor = star.color;
    homeCtx.shadowBlur = star.radius * 3;
    homeCtx.beginPath();
