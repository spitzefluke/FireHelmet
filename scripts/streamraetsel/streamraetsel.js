...
/* ------------------------------------------------------
   ZAHLEN-KETTE
   Nur die aktuelle Zahl ist eingebbar, der Rest ist gesperrt.
   Bei falscher Eingabe steigt die Wartezeit mit jedem Fehler:
   5s, 10s, 20s, 40s ... (verdoppelt sich, max. 2 Minuten).
------------------------------------------------------ */
const numberChainState = {
  currentIndex: 0,
  wrongAttempts: 0,
  lockedUntil: 0,
};

function getNumberChainWaitSeconds(attempts) {
  const seconds = 5 * Math.pow(2, attempts - 1);
  return Math.min(seconds, 120);
}

/* ------------------------------------------------------
   AUDIO BEI GELÖSTEN ZAHLEN (max. 3x abspielbar)
------------------------------------------------------ */
const NUMBER_CHAIN_MAX_PLAYS = 3;

function getNumberChainPlayCount(index) {
  return parseInt(localStorage.getItem(`numberChainAudioPlays_${index}`) || "0", 10);
}

function buildNumberChainAudioButton(item, index) {
  if (!item.audio) return "";

  const playsLeft = Math.max(0, NUMBER_CHAIN_MAX_PLAYS - getNumberChainPlayCount(index));
  const disabled = playsLeft <= 0 ? "disabled" : "";
  const label = playsLeft > 0 ? `🔊 Anhören (${playsLeft}x übrig)` : "🔇 Aufgebraucht";

  return `<button type="button" class="number-chain-audio-btn" ${disabled} onclick="playNumberChainAudio(${index})">${label}</button>`;
}

function playNumberChainAudio(index) {
  const item = streamRaetselConfig.numberChain[index];
  if (!item || !item.audio) return;

  const playCount = getNumberChainPlayCount(index);
  if (playCount >= NUMBER_CHAIN_MAX_PLAYS) return;
  ...
  localStorage.setItem(`numberChainAudioPlays_${index}`, String(playCount + 1));
  refreshNumberChain();
}

function buildNumberChainHtml() {
  if (typeof streamRaetselConfig === "undefined" || !streamRaetselConfig.numberChain) return "";

  let slots = "";

  streamRaetselConfig.numberChain.forEach((item, i) => {
    if (i < numberChainState.currentIndex) {
      // bereits gelöst
      slots += `
        <div class="number-chain-slot number-chain-solved">
          <span class="number-chain-index">${i + 1}</span>
          <span class="number-chain-value">${item.answer}</span>
          <span class="number-chain-check">✅</span>
          ${buildNumberChainAudioButton(item, i)}
        </div>
      `;
    } else if (i === numberChainState.currentIndex) {
      // aktiv, eingebbar
      slots += `
        <div class="number-chain-slot number-chain-active" id="number-chain-slot-${i}">
          <span class="number-chain-index">${i + 1}</span>
          <input type="text" class="code-input number-chain-input" id="number-chain-input-${i}"
                 placeholder="?" autocomplete="off"
                 onkeydown="if(event.key==='Enter') submitNumberChain(${i})">
          <button type="button" class="code-button" onclick="submitNumberChain(${i})">OK</button>
        </div>
      `;
    } else {
      // gesperrt
      slots += `
        <div class="number-chain-slot number-chain-locked">
          <span class="number-chain-index">${i + 1}</span>
          <span class="number-chain-lock-icon">🔒</span>
        </div>
      `;
    }
  });
  ...
  return `
    <div class="number-chain-wrap">
      ${slots}
    </div>
    <p id="number-chain-message" class="code-message"></p>
  `;
}

function submitNumberChain(index) {
  if (index !== numberChainState.currentIndex) return; // nur die aktive Zahl zählt

  const messageEl = document.getElementById("number-chain-message");
  const input = document.getElementById(`number-chain-input-${index}`);
  if (!input || !messageEl) return;

  const now = Date.now();
  if (now < numberChainState.lockedUntil) {
    const waitLeft = Math.ceil((numberChainState.lockedUntil - now) / 1000);
    messageEl.classList.remove("code-success");
    messageEl.classList.add("code-error");
    messageEl.textContent = `⏳ Zu viele Fehlversuche - warte noch ${waitLeft} Sekunde(n).`;
    return;
  }
  ...
  if (guess === item.answer) {
    numberChainState.wrongAttempts = 0;
    numberChainState.currentIndex++;
    ...
    numberChainState.wrongAttempts++;
    const waitSeconds = getNumberChainWaitSeconds(numberChainState.wrongAttempts);
    numberChainState.lockedUntil = now + waitSeconds * 1000;

    messageEl.classList.add("code-error");
    messageEl.textContent = `❌ Falsch. Nächster Versuch in ${waitSeconds} Sekunde(n) möglich.`;
    input.value = "";
  }
}

function refreshNumberChain() {
  const wrap = document.querySelector(".number-chain-wrap");
  if (!wrap) return;

  wrap.outerHTML = buildNumberChainHtml();
}
