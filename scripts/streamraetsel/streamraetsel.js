...
function playComboAudio(audioPath) {
...
function showComboMessage(text, type) {
...
function checkComboDigit() {
  ...
  if (currentDigit === correctDigit) {
    ...
    showComboMessage(digitInfo && digitInfo.message ? digitInfo.message : "✅ Richtig!", "success");

    if (digitInfo && digitInfo.audio) {
      playComboAudio(digitInfo.audio);
    }
